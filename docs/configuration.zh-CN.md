...
配置页展示公开参数和密钥是否存在，不回传密钥原文。更新时可以缺省 `client_secret`；只要 Client ID、协议、Issuer、预设、令牌端点和令牌认证方式没有变化，插件会自动保留原密钥。输入新值表示替换，显式提交 `clear_secret: true` 才会清除。修改上述安全边界字段后仍需重新填写密钥，避免把已有凭据带到另一个客户端或令牌端点。

以上行为也适用于高级 JSON 更新。`clear_secret` 只是更新指令，标准化保存后不会留在配置中。写入使用版本检查，发生冲突后应刷新并重新应用修改。
