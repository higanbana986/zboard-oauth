...
每个 `vX.Y.Z` 标签只发布本插件，清单和运行时都必须报告 `X.Y.Z`。当前待发布版本为 `v0.0.2`。
...
签名环境配置完成后，在 `main` 上选择已审核且清单与运行时版本一致的提交，当前发行版本为 `v0.0.2`。

- 从该提交创建 `release/v0.0.2` 分支，即可在没有既有标签时启动发布。检查与签名成功后，工作流在已验证的源码提交创建标签和 Release。
- 也可以推送对应的 `v0.0.2` 标签，或在 Actions → Release → Run workflow 中选择已审核的 main 提交或发行标签，并填写 `v0.0.2`。
