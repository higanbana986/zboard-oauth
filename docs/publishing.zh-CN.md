...
每个 SemVer 标签只发布本插件。开发版使用 `vX.Y.Z-dev.N`，候选版使用 `vX.Y.Z-rc.N`，正式版使用 `vX.Y.Z`；清单和运行时必须报告去掉前导 `v` 后的同一版本。当前开发版本为 `v0.0.2-dev.202609111242`。
...
签名环境配置完成后，在 `main` 上选择已审核且清单与运行时版本一致的提交，当前发行版本为 `v0.0.2-dev.202609111242`。

- 从该提交创建 `release/v0.0.2-dev.202609111242` 分支，即可在没有既有标签时启动发布。检查与签名成功后，工作流在已验证的源码提交创建标签和 prerelease。
