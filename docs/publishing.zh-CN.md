...
每个 SemVer 标签只发布本插件。开发版使用 `vX.Y.Z-dev.N`，候选版使用 `vX.Y.Z-rc.N`，正式版使用 `vX.Y.Z`；清单和运行时必须报告去掉前导 `v` 后的同一版本。当前开发版本为 `v0.0.2-dev.202609111158`。
...
发布流程核对标签属于 main 历史，在不接触发布密钥的阶段运行检查，再构建五个平台包、验证元数据、生成校验文件和 `marketplace-entry.json`，最后发布 GitHub Release。开发版和候选版会标记为 GitHub prerelease。已有发行版本不覆盖。失败的草稿发行保留以供检查；重试前删除不完整草稿。未配置签名参数时，创建标签不会成功发布。

## 触发发布

签名环境配置完成后，在 `main` 上选择已审核且清单与运行时版本一致的提交，当前发行版本为 `v0.0.2-dev.202609111158`。

- 从该提交创建 `release/v0.0.2-dev.202609111158` 分支，即可在没有既有标签时启动发布。检查与签名成功后，工作流在已验证的源码提交创建标签和 prerelease。
- 也可以推送对应标签，或在 Actions → Release → Run workflow 中选择已审核的 main 提交或发行标签，并填写完整 SemVer 标签。
