(() => {
  ...
  function row(binding) {
    ...
    for (const [label, value] of [['来源', binding.publisher], ['Issuer', binding.issuer], ['第三方账号标识', binding.subject], ['绑定时间', new Date(binding.created_at).toLocaleString()]]) { const line = document.createElement('small'); line.textContent = `${label}：${value || '—'}`; details.append(line); }
