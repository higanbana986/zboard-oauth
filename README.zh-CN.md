...
**插件 ID：** `zboard.oauth` · **发行版本：** `v0.0.2-dev.202609111242`

插件自身提供登录入口、账户安全中的绑定/解绑界面，以及管理员用户详情中的关联账号信息；ZBoard 只提供受控插槽、身份映射、权限校验、会话和审计服务。
...
宿主需实现插件要求的身份插槽、受控身份桥、公开配置投影和外部账户注册流程。Manifest 当前声明 `>=0.0.2 <0.1.0`、插件协议 `1` 和页面桥 `1`；除宿主显示版本外，还应确认 API 支持情况。

部署站点需要公开 HTTPS 根地址，并在提供方登记应用回调地址。当提供方未返回已验证邮箱、新用户需要补充邮箱验证时，站点还需配置 SMTP。

安装包供应状态见[发行页面](https://github.com/higanbana986/zboard-oauth/releases)。构建目标包含 Linux、macOS 和 Windows。macOS 宿主启动曾遇到 Gatekeeper 拒绝，稳定平台运行及生产签名、公证仍待验证。
...
提交 [Issue](https://github.com/higanbana986/zboard-oauth/issues) 时，请提供插件版本、宿主构建、平台和脱敏复现步骤。安全漏洞按[安全政策](SECURITY.zh-CN.md)报告。
