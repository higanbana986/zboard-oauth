...
async function fixture(initial=[],failSave=false){
 ...
 const context={document:{querySelector:()=>({}),getElementById:id=>elements.get(id),createElement:element,documentElement:{scrollHeight:900},body:{}},Number,Error,addEventListener(){},oauthBridge:{observeSize:()=>()=>{},resize(){},async request(type,body){calls.push({type,body:body&&JSON.parse(JSON.stringify(body))});if(type==='context.load')return{plugin_id:'zboard.oauth',surface:'admin',page_id:'settings'};if(type==='config.load')return{revision,configured:true,config:{providers}};if(type==='config.save'){if(failSave)throw new Error('conflict');revision++;const previous=providers;providers=body.config.providers.map(p=>({...p,has_secret:!!p.client_secret||(!p.clear_secret&&!!previous.find(old=>old.id===p.id)?.has_secret),client_secret:undefined,keep_secret:undefined,clear_secret:undefined}));return{revision,configured:true};}return{healthy:true};}}};
...
test('editing one provider preserves other entries and retains secrets',async()=>{
 ...
 await e.get('save').listeners.click({preventDefault(){}});const p=calls.find(c=>c.type==='config.save').body.config.providers;assert.equal(p.length,2);assert.equal(p[0].disabled,true);assert.equal('client_secret'in p[0],false);assert.equal('keep_secret'in p[0],false);assert.equal('client_secret'in p[1],false);assert.equal('keep_secret'in p[1],false);
});
test('clearing a stored secret is explicit',async()=>{
 const {elements:e,calls}=await fixture([{id:'github',name:'GitHub',preset:'github',client_id:'gh',has_secret:true}]);e.get('providers').children[0].listeners.click();e.get('clear-secret').checked=true;
 await e.get('save').listeners.click({preventDefault(){}});const p=calls.find(c=>c.type==='config.save').body.config.providers[0];assert.equal(p.clear_secret,true);assert.equal('client_secret'in p,false);
});
